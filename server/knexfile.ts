// env must be loaded before anything reads process.env (see src/env.ts).
import './src/env';
import path from 'path';
import type { Knex } from 'knex';

// After `tsc` the compiled knexfile lives at dist/knexfile.js and the
// migrations at dist/src/db/migrations/*.js — resolve accordingly.
const isCompiled = __filename.endsWith('.js');

const config: Knex.Config = {
  client: 'pg',
  connection:
    process.env.DATABASE_URL || 'postgres://oblidesk:changeme@localhost:5432/oblidesk',
  searchPath: ['public'],
  migrations: {
